...
for event in tree:
    ...
    if sum(event.parton_top_isHadronic) == 2 and len(event.parton_top_isHadronic) == 4:
        ...
        lt_count = 0
        for t in range(4):
            if event.parton_top_isHadronic[t] == 0:
                if lt_count == 0:
                    b1.append(event.b_recoj_index[t]+4 if event.b_recoj_index[t] != -1 else -1)
        
                    if event.wd1_el_index[t] != -1:
                        l1.append(event.wd1_el_index[t])
                    elif event.wd2_el_index[t] != -1:
                        l1.append(event.wd2_el_index[t])
                    elif event.wd1_mu_index[t] != -1:
                        l1.append(event.wd1_mu_index[t] + event.nElectrons)
                    elif event.wd2_mu_index[t] != -1:
                        l1.append(event.wd2_mu_index[t] + event.nElectrons)
                    else:
                        l1.append(-1)
                    lt_count = 1
                if lt_count == 1:
                    b2.append(event.b_recoj_index[t]+4 if event.b_recoj_index[t] != -1 else -1)
                    ...
                    b3.append(event.b_recoj_index[t]+4 if event.b_recoj_index[t] != -1 else -1)
                    ...
                    b4.append(event.b_recoj_index[t]+4 if event.b_recoj_index[t] != -1 else -1)
...
l1_data = t1.create_dataset('l', data=l1)
