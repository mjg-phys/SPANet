...
hf = h5py.File("/eos/user/d/dreiter/SPANet/predict/ttbar_testing_output2.h5", 'r')

output_folder = "/eos/user/d/dreiter/SPANet/predict/ttbar_hists/"

def t_m(pt, eta, phi, mass, b_index, j1_index, j2_index):
    # print(b_index-2)
    # print(j1_index-2)
    # print(j2_index-2)
    # print("njets " + str(len(event.jet_pt_NOSYS)))
    b = Math.PtEtaPhiMVector()
    b.SetCoordinates(pt[b_index]*1000, eta[b_index], phi[b_index], mass[b_index]*1000)

    j1 = Math.PtEtaPhiMVector()
    j1.SetCoordinates(pt[j1_index]*1000, eta[j1_index], phi[j1_index], mass[j1_index]*1000)

    j2 = Math.PtEtaPhiMVector()
    j2.SetCoordinates(pt[j2_index]*1000, eta[j2_index], phi[j2_index], mass[j2_index]*1000)
            
    t = b + j1 + j2
    return t.M()

def W_m(pt, eta, phi, mass, j1_index, j2_index):
    j1 = Math.PtEtaPhiMVector()
    j1.SetCoordinates(pt[j1_index]*1000, eta[j1_index], phi[j1_index], mass[j1_index]*1000)
...
mass = np.array(hf.get('INPUTS/Source/mass'))


for i in range(len(b1)): 

    tm1 = t_m(pt[i], eta[i], phi[i], mass[i], b1[i], q11[i], q12[i])
    tm2 = t_m(pt[i], eta[i], phi[i], mass[i], b2[i], q21[i], q22[i])
    
    wm1 = W_m(pt[i], eta[i], phi[i], mass[i], q11[i], q12[i])
    wm2 = W_m(pt[i], eta[i], phi[i], mass[i], q21[i], q22[i])

    # tm1 = float(mass[i][b1[i]] + mass[i][q11[i]] + mass[i][q12[i]])*1000
    # tm2 = float(mass[i][b2[i]] + mass[i][q21[i]] + mass[i][q22[i]])*1000

    # wm1 = float(mass[i][q11[i]] + mass[i][q12[i]])*1000
    # wm2 = float(mass[i][q21[i]] + mass[i][q22[i]])*1000
